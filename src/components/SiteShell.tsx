  ...
  return (
    <div className="min-h-screen overflow-x-clip bg-background text-foreground">
      ...
      <header className="site-header">
        ...
        <nav className="desktop-nav" aria-label="Main navigation">
          ...
          <div className="nav-dropdown">
            <button className={`nav-link dropdown-toggle ${isServiceActive ? "active" : ""}`} onMouseEnter={() => setServicesOpen(true)} onMouseLeave={() => setServicesOpen(false)}>
              Services <ChevronDown size={16} />
            </button>
            {servicesOpen && (
              <div className="dropdown-menu" onMouseEnter={() => setServicesOpen(true)} onMouseLeave={() => setServicesOpen(false)}>
                {serviceLinks.map(([to, label]) => (
                  <Link key={to} to={to} className={`dropdown-item ${pathname === to ? "active" : ""}`}>
                    {label}
                  </Link>
                ))}
              </div>
          ...
          <div className="mobile-dropdown">
            ...
            {servicesOpen && (
              <div className="mobile-dropdown-menu">
                {serviceLinks.map(([to, label]) => (
                  <Link key={to} to={to} onClick={() => { setOpen(false); setServicesOpen(false); }} className="dropdown-item">
                    {label}
                  </Link>
                ))}
              </div>
      ...
      <footer className="site-footer">
        ...
        <div><strong>Therapy services</strong>{serviceLinks.map(([to, label]) => <Link key={to} to={to}>{label}</Link>)}</div>
