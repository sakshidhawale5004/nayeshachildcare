...
export function HomePage() {
  ...
  return <>
    ...
    <section className="story-split section-wrap reveal reveal-delay-1">
      <div className="image-stage"><img src={homeChildrenDifferenceImage} alt="Child receiving gentle care from a parent" loading="lazy" width={1408} height={1056} /><span className="image-note">Care designed around curiosity</span></div>
    ...
    <section className="quote-section section-wrap reveal reveal-delay-1"><img src={lifestyleImage} alt="Mother and young son walking together in a park" loading="lazy" width={1408} height={1056} /><blockquote>“The best pediatric care doesn’t only treat a symptom. It helps a family feel informed, capable and supported.”<cite>Nayesha care philosophy</cite></blockquote></section>
    <Callout />
  </>;
}
...
export function PageHero({ eyebrow, title, intro, image, alt }: { eyebrow: string; title: string; intro: string; image: string; alt: string }) {
  return <section className="page-hero section-wrap reveal"><div className="reveal-delay-1"><Eyebrow>{eyebrow}</Eyebrow><h1>{title}</h1><p>{intro}</p><NayeshaButton to="/contact">Talk to our care team</NayeshaButton></div><div className="page-hero-image reveal-delay-2"><img src={image} alt={alt} width={1408} height={1056} /></div></section>;
}
...
export function ContentPage({ eyebrow, title, intro, image, alt, highlights, sections, visuals }: { eyebrow:string; title:string; intro:string; image:string; alt:string; highlights:{title:string;text:string}[]; sections:{title:string;text:string;bullets:string[]}[]; visuals?: string[] }) {
  return <><PageHero {...{eyebrow,title,intro,image,alt}} /><section className="highlight-strip section-wrap reveal">{highlights.map((x,i)=><div className={`reveal reveal-delay-${Math.min(i + 1, 4)}`} key={x.title}><b>0{i+1}</b><h3>{x.title}</h3><p>{x.text}</p></div>)}</section><section className="detail-stack section-wrap">{sections.map((s,i)=><article key={s.title} className={i%2 ? "detail-row reverse reveal reveal-delay-1":"detail-row reveal reveal-delay-1"}><div className="detail-visual reveal-delay-2"><img src={visuals?.[i] ?? [childrenImage,checkupImage,clinicImage,lifestyleImage][i%4]} alt="Nayesha pediatric care experience" loading="lazy" width={1408} height={1056} /></div><div className="reveal-delay-3"><Eyebrow>{String(i+1).padStart(2,"0")} · Care detail</Eyebrow><h2>{s.title}</h2><p>{s.text}</p><ul className="check-list">{s.bullets.map(b=><li key={b}>{b}</li>)}</ul></div></article>)}</section><Callout /></>;
}
